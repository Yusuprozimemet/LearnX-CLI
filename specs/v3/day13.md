...
```powershell
py -m pytest tutor/tests/audio/ -v   # scoped — all green
...
```
Writes (new):
  audio/<session>/tutorial.timing.json   ← written once per /generate run
...
```json
{
  ...
  "units": {
    "1": [
      {"line_index": 0, "speaker": "ALEX", "text": "What is the difference...", "start_ms": 0,    "end_ms": 3240},
      {"line_index": 1, "speaker": "ALEX", "text": "Think of it this way...",  "start_ms": 3740, "end_ms": 7180},
      {"line_index": 2, "speaker": "MAYA", "text": "So an interface is...",    "start_ms": 7680, "end_ms": 9520}
    ],
    "2": [...]
  }
}
...
**Rules:**
- Keys in `"units"` are unit numbers as plain string integers (`"1"`, `"2"`, …).
- Unit 0 (intro) and unit -1 (outro) are excluded — the video pipeline processes
  teaching units only.
- Offsets are relative to each unit's MP3, not the full-session audio.
- `start_ms` of line N equals `end_ms` of line N-1 plus silence gap. No overlaps;
  every millisecond in the unit MP3 is accounted for.
...
Call `_concat_with_silence(group, capture_timing=True)` for teaching units
(`unit_num >= 1`). Collect results into a dict keyed by unit number string, then
write `tutorial.timing.json` before exporting the full audio.
...
`audio_builder.py` is currently 106 lines. After this change: approximately
...
- Sessions generated before Day 13 have no `tutorial.timing.json`. The video pipeline
  checks for presence before using it; absence triggers proportional fallback (Day 16).
...
- [ ] `"units"` keys are plain string integers (`"1"`, `"2"`, …); no `"unit_1"` prefix
- [ ] Teaching units 1..N present in `"units"`; unit 0 and -1 excluded
...
- [ ] Same-speaker gaps match `SILENCE_BREATH_MS`; speaker-change gaps match `SILENCE_TURN_MS`
...
- [ ] Timing file is not written if audio assembly raises before reaching the write step
...
- `test_timing_file_written_after_build` — assert `tutorial.timing.json` exists after `build()`
- `test_timing_keys_are_string_integers` — keys are `"1"`, `"2"`, not `"unit_1"`
...
- `test_timing_version_field_is_1` — top-level `"version"` is `1`
- `test_intro_and_outro_excluded_from_timing` — `"0"` and `"-1"` are not keys
