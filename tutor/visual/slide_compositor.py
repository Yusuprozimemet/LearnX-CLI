...
def compose_all(
    ...
    for spec in unit_specs:
        ...
        paths.append(compose_concept_slide(spec, diag_png, output_dir / f"{i:02d}_concept.png", total))
...
def compose_title_card(spec: VisualSpec, output_path: Path) -> Path:
    ...
    draw.rounded_rectangle(
        [lx_x0, lx_y0, lx_x0 + lx_size, lx_y0 + lx_size],
        radius=20, fill=ACCENT_CYAN
    )
    ...
    draw.text((CONTENT_LEFT, FOOTER_BAR_Y + 28), spec.doc_source or "", font=font_meta, fill=TEXT_SECONDARY)
    lx_label = "LearnX v2"
    bbox = draw.textbbox((0, 0), lx_label, font=font_meta)
    draw.text((CONTENT_RIGHT - (bbox[2] - bbox[0]), FOOTER_BAR_Y + 28), lx_label, font=font_meta, fill=TEXT_SECONDARY)
...
def compose_hook_slide(spec: VisualSpec, output_path: Path, total: int) -> Path:
    ...
    draw.rounded_rectangle(
        [CONTENT_LEFT - 16, panel_y0 - 20, panel_x1, panel_y0 + 280],
        radius=12, fill=BG_PANEL
    )
    draw.rectangle(
        [CONTENT_LEFT - 16, panel_y0 - 20, CONTENT_LEFT - 6, panel_y0 + 280],
        fill=ACCENT_AMBER
    )
    ...
    if spec.key_points:
        ...
        for pt in spec.key_points[:3]:
            ...
            draw.text((CONTENT_LEFT + 4, learn_y), f"+ {pt_lines[0]}", font=font_pt, fill=TEXT_PRIMARY)
            learn_y += 42
...
def compose_memory_slide(spec: VisualSpec, output_path: Path, total: int) -> Path:
    ...
    draw.text((CONTENT_LEFT, 22), f"UNIT {spec.unit_index} / {total}  -  REMEMBER THIS", font=font_bar, fill=TEXT_SECONDARY)
    ...
    if spec.analogy:
        ...
        draw.text((CONTENT_RIGHT - 800, text_y), analogy_lines[0] if analogy_lines else "", font=font_anal, fill=TEXT_SECONDARY)
...
def compose_outro_card(spec: VisualSpec, output_path: Path, session_label: str) -> Path:
    ...
    draw.text((cx - (bbox[2] - bbox[0]) // 2, 298), spec.session_stats, font=font_stats, fill=TEXT_SECONDARY)
